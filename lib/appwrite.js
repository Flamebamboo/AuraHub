...
async function createUserDocument(accountData, retryCount = 0) {
  ...
  try {
    ...
    console.log(
      "Attempting to create document with data:",
...
export async function createUser(email, password, username) {
  try {
    ...
    if (error.message.includes("Missing required attribute")) {
      ...
      console.log(
        "Required fields: userId, email, username, createdAt, updatedAt"
      );
      console.log(
        "Please verify all required fields are present in your schema"
      );
    }
    throw error;
  }
